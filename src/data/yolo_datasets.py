...
class AlbumentationYOLODataset(YOLODataset):
    ...
    def __getitem__(self, index):
        ...
        if 'bboxes' in item and len(item['bboxes']) > 0:
            bboxes = item['bboxes'].numpy() if hasattr(item['bboxes'], 'numpy') else item['bboxes']
            
            # Extract class labels
            if 'cls' in item:
                class_labels = item['cls'].numpy() if hasattr(item['cls'], 'numpy') else item['cls']
            ...
            try:
                transformed = self.albumentations_transform(image=img, bboxes=[], class_labels=[])
                img = transformed['image']
            except:
                pass
